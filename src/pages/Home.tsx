      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-0 border-b border-soft-sepia pb-12">
        <section className="md:col-span-6 bg-transparent p-6 md:p-10 pl-0 flex flex-col justify-between">
          <div>
            ...
            <p className="text-charcoal-light rtl:font-arabic leading-relaxed mb-10 text-base rtl:text-xl max-w-xl pl-6 rtl:pl-0 rtl:pr-6">
              {language === "ar"
                ? bioAr || t("home.bio")
                : bio || t("home.bio")}
            </p>

            <div className="flex items-center gap-6 mb-8 pl-6 rtl:pl-0 rtl:pr-6">
              <a
                href={resumeUrl}
                target="_blank"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#2C2C2C] text-[#F9F7F2] dark:bg-[#F9F7F2] dark:text-[#1A1918] rounded-sm text-xs rtl:text-base uppercase tracking-widest rtl:tracking-normal rtl:font-arabic font-semibold hover:bg-opacity-90 transition-colors shadow-sm"
              >
                {t("home.viewResume")}
              </a>
            ...
            {topSkills.length > 0 && (
              <div className="pl-6 mt-12 flex items-center mb-8 relative">
                ...
                <div className="flex-1 ml-4 rtl:mr-4 rtl:ml-0 hidden md:block">
                  <h3 className="text-[10px] uppercase tracking-[0.2em] font-bold mb-4 opacity-40">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {topSkills.map((tag, idx) => (
                      <span
                        key={`${tag.name}-${idx}`}
                        className="text-xs rtl:text-sm rtl:font-arabic rtl:px-3 px-2.5 py-1.5 bg-soft-sepia/20 text-charcoal-light font-medium rounded-sm border border-soft-sepia/20 hover:bg-soft-sepia/40 transition-colors cursor-default"
                      >
                        {tag.name}
                      </span>
                    ))}
          ...
          <div className="flex flex-col -space-y-16 items-center w-full max-w-md pt-8 pb-12">
            {heroImage1 ? (
              <HeroImage
                src={heroImage1}
                alt="Hero 1"
                className="w-[280px] h-[186px] md:w-[320px] md:h-[213px] object-cover rounded-sm rotate-[-4deg] shadow-md border-[6px] border-warm-white z-30 transform-gpu bg-soft-sepia/20"
              />
            ) : (
              <div className="w-[280px] h-[186px] md:w-[320px] md:h-[213px] bg-charcoal rounded-sm rotate-[-4deg] shadow-md border-[6px] border-warm-white z-30 flex items-center justify-center font-serif text-2xl text-warm-white/20 transform-gpu">
                Photo 1
              </div>
            )}
            {heroImage2 ? (
              <HeroImage
                src={heroImage2}
                alt="Hero 2"
                className="w-[280px] h-[186px] md:w-[320px] md:h-[213px] object-cover rounded-sm rotate-[5deg] shadow-md border-[6px] border-warm-white z-20 ml-20 rtl:mr-20 rtl:ml-0 transform-gpu bg-soft-sepia/20"
              />
            ) : null}
            {heroImage3 ? (
              <HeroImage
                src={heroImage3}
                alt="Hero 3"
                className="w-[280px] h-[186px] md:w-[320px] md:h-[213px] object-cover rounded-sm rotate-[-3deg] shadow-md border-[6px] border-warm-white z-10 mr-16 rtl:ml-16 rtl:mr-0 transform-gpu bg-soft-sepia/20"
              />
            ) : null}
          </div>
        </section>
