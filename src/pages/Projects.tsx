...
const getProjectIcon = (iconName?: string | null) => {
  ...
  if (iconName in FaIcons) {
    const Icon = (FaIcons as any)[iconName]; return <Icon className="w-6 h-6" />;
  }
  if (iconName in SiIcons) {
    const Icon = (SiIcons as any)[iconName]; return <Icon className="w-6 h-6" />;
  }
  if (iconName in LucideIcons) {
    const Icon = (LucideIcons as any)[iconName]; return <Icon className="w-5 h-5" strokeWidth={1.5} />;
  }
...
export default function Projects() {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[50vh]"><div className="w-8 h-8 border-2 border-soft-sepia border-t-accent rounded-full animate-spin"></div></div>;
    ...
    <div className="space-y-12 animate-fade-in w-full">
      <div className="border-b border-soft-sepia pb-6 mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-charcoal-light text-sm max-w-lg leading-relaxed">
            A selection of recent development work, software engineering projects, and technical experiments.
          </p>
        </div>
        <div className="text-[10px] uppercase tracking-widest font-mono text-muted flex items-center gap-2">
          <Code2 className="w-4 h-4" strokeWidth={1.5} /> {projects.length} Repositories
        </div>
      </div>
            ...
            >
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-2 mt-0.5">
                      {project.githubUrl && project.githubUrl !== "#" && (
                        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-charcoal transition-colors" aria-label="Source Code">
                          <Github className="w-4 h-4" strokeWidth={1.5} />
                        </a>
                      )}
                      {project.liveUrl && project.liveUrl !== "#" && (
                        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-accent transition-colors" aria-label="Live Site">
                          <ExternalLink className="w-4 h-4" strokeWidth={1.5} />
                        </a>
                      )}
                    </div>
                  </div>
                  <p className="text-charcoal-light text-xs leading-relaxed mt-1.5 font-light">
              ...
              <div className="flex flex-wrap gap-2 mt-auto pt-3 border-t border-soft-sepia/50 w-full">
                {tags.length > 0 ? tags.map((tag) => (
                  <span key={tag} className="group/tag flex items-center overflow-hidden px-2 py-1.5 bg-soft-sepia/20 text-charcoal-light rounded-sm font-medium hover:bg-soft-sepia/40 transition-all duration-300 max-w-[2rem] hover:max-w-[12rem] cursor-default box-border">
      ...
      {projects.length === 0 && (
        <div className="py-20 text-center flex flex-col items-center justify-center text-charcoal-light border border-dashed border-soft-sepia rounded-sm">
          <Folder className="w-10 h-10 mb-4 text-muted opacity-50" strokeWidth={1} />
